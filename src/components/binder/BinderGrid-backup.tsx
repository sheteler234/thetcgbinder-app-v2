...
const BinderGrid: React.FC<BinderGridProps> = ({ 
  ...
  // Ensure we have at least the number of pages specified
  while (pagesData.length < externalTotalPages) {
    pagesData.push([]);
  }
  ...
  const goToPage = (pageIndex: number) => {
    if (pageIndex !== currentPage && pageIndex >= 0 && pageIndex < pagesData.length) {
  ...
  return (
    <div className="relative overflow-visible">
      ...
      <div className="relative bg-gradient-to-br from-amber-900 via-amber-800 to-amber-900 p-8 rounded-lg shadow-2xl border-4 border-amber-700" style={{ overflow: 'visible' }}>
        
        {/* Yugi Character - Standing above the Brown Binder */}
        <div className="absolute -top-56 right-8 z-20 pointer-events-none">
          <motion.img
            src="/yugi-character.png"
            alt="Yugi Muto"
            className="w-48 h-64 object-contain drop-shadow-xl"
        ...
        {/* Page content area */}
        <div className="ml-12 rounded shadow-lg relative" style={{ aspectRatio: '8.5/11', overflow: 'visible' }}>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {renderGrid(pagesData[currentPage] || [])}
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="mt-6 flex justify-between items-center">
          ...
          <div className="ml-12 flex items-center gap-4">
            <button
              ...
              className="p-2 bg-amber-600 text-white rounded-full disabled:bg-gray-400 disabled:cursor-not-allowed hover:bg-amber-700 transition-colors"
            >
          ...
          <div className="text-amber-200 font-medium">
            {currentPage + 1} of {pagesData.length}
          </div>
        </div>
      </div>
    </div>
